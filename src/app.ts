import { WorkspaceData, Task, TemplateCard } from './core/models.js';
...
let workspace: WorkspaceData = loadWorkspace();
let active: ModuleName = 'Command Center';
...
const render = () => {
  ...
  document.querySelectorAll<HTMLButtonElement>('.nav-btn').forEach((btn) => btn.onclick = () => { active = btn.dataset.module as ModuleName; render(); });
  attachActions();
};

function renderCards(tasks: Task[]) {
  if (!tasks.length) return '<div class="empty">No items yet.</div>';
...
function renderModule(module: ModuleName, focus8: Task[], queue92: Task[]): string {
  switch (module) {
    case 'Command Center':
    ...
    case 'Quadrant Board':
      return `<section><h2>Quadrant Board</h2><div class="quad">${(['Q1-Urgent-Important','Q2-Strategic','Q3-Delegate','Q4-Eliminate'] as const).map(q=>`<div class="quad-col"><h3>${q}</h3>${renderCards(workspace.tasks.filter(t=>!t.archived&&t.quadrant===q))}</div>`).join('')}</div></section>`;
...
function attachActions() {
  ...
  if (taskForm) taskForm.onsubmit = (e) => {
    ...
    const base = {
      ...
      zone: String(data.get('zone')) as Task['zone'],
  ...
  if (ideaForm) ideaForm.onsubmit = (e) => {
    e.preventDefault();
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-convert-idea]').forEach((btn) => btn.onclick = () => {
    const idea = workspace.ideas.find((i) => i.id === btn.dataset.convertIdea);
    if (!idea) return;
    const base = { title: idea.title, description: idea.notes || 'Created from idea capture.', zone: 'Build Things' as const, impact: 7, urgency: 6, strategic: 7, effort: 4, repeatability: 5 };
    const score8 = scoreEightPercent(base);
    workspace.tasks.unshift({ ...base, id: id(), score8, quadrant: calculateQuadrant(base), automationRoute: routeAutomation({ ...base, score8 }), archived: false, createdAt: new Date().toISOString() });
    idea.archived = true;
    persist();
  ...
  if (agentForm) agentForm.onsubmit = (e) => {
    e.preventDefault();
  ...
  if (workflowForm) workflowForm.onsubmit = (e) => {
    e.preventDefault();
  ...
  if (templateForm) templateForm.onsubmit = (e) => {
    e.preventDefault();
    const fd = new FormData(templateForm);
    workspace.templates.unshift({ id: id(), title: String(fd.get('title')), type: String(fd.get('type')) as TemplateCard['type'], content: String(fd.get('content') ?? ''), archived: false });
    persist();
  };

  document.querySelectorAll<HTMLButtonElement>('[data-dup-template]').forEach((btn) => btn.onclick = () => {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-toggle-template]').forEach((btn) => btn.onclick = () => {
  ...
  document.querySelector<HTMLInputElement>('#import-json')?.addEventListener('change', async (e) => {
    const file = (e.target as HTMLInputElement).files?.[0];
    if (!file) return;
    const text = await file.text();
    workspace = JSON.parse(text) as WorkspaceData;
    persist();
  });
