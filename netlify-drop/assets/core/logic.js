...
export function calculateQuadrant(task) {
    if (task.urgency >= 7 && task.impact >= 7)
        return 'Q1-Urgent-Important';
    if (task.strategic >= 7 && task.impact >= 6)
        return 'Q2-Strategic';
    if (task.repeatability >= 6 || task.urgency <= 4)
        return 'Q3-Delegate';
    return 'Q4-Eliminate';
}
export function scoreEightPercent(task) {
...
export function routeAutomation(task) {
    if (task.score8 >= 8)
        return 'Keep Human';
    if (task.repeatability >= 8)
        return 'Automate';
    if (task.strategic >= 7 && task.repeatability >= 5)
        return 'Systemize';
    if (task.effort <= 4)
        return 'Template';
    return 'Delegate';
}
export function topEightPercentTasks(tasks) {
...
export function automationQueue(tasks) {
